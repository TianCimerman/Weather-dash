...
export default function FeederLogs() {
  ...
  useEffect(() => {
    const loadLogs = () => {
      fetch(`/api/feeder/logs?lines=10`, { cache: "no-store" })
        .then((r) => r.json())
        .then((data) => {
  ...
  return (
    ...
    style={{ backgroundColor: "hsl(218, 46%, 8%)" }}
      ...
      "
    >
        <p className="text-white text-2xl hss:text-base font-bold">Last Feeding</p>
          ...
          {status && status.logs && status.logs.length > 0 ? (
            (() => {
              const relevantLogs = status.logs.filter(log => {
                ...
                return isFeeding;
              }).reverse().slice(0, 1); // Only show the last feeding event
              ...
              return relevantLogs.length > 0 ? (
                <ul className="text-left space-y-3"> 
                  {relevantLogs.map((log, idx) => {
                    ...
                    return (
                      <li key={idx} className={`${bgColor} ${borderColor} border rounded-lg p-2 transition-all`}>
                          <div className="flex items-start gap-3">
                            ...
                            <div className="text-white text-center w-full text-base hss:text-xs leading-relaxed ml-[-1.5rem]">
                            {displayMessage}
                            </div>
                            </div>
                          </div>
